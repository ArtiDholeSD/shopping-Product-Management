...
const addToCart = async function (req, res) {
  try {
    ...
    if (cartCheck == null) {
      ...
      //----------cheking if product already present and add total without adding another object in items
      for (let i = 0; i < cartCheck.items.length; i++) {
        console.log(cartCheck.items[i].productId)
        console.log(items[i].productId)

        if (cartCheck.items[i].productId == items[i].productId) {
